...
## Gotchas
- The suite is green: `flutter test` (90) + `cd backend && npm test` (124). `test/widget_test.dart` holds real translation-parity tests — **every new string must be added to BOTH `en_US` and `ar_AR` maps** in `lib/utils/translations.dart` or the suite fails.
