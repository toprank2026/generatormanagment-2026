...
## Gotchas
- The suite is green: `flutter test` (90) + `cd backend && npm test` (123). `test/widget_test.dart` holds real translation-parity tests — **every new string must be added to BOTH `en_US` and `ar_AR` maps** in `lib/utils/translations.dart` or the suite fails.
- ⚠️ **SQLite is `version: 11`** (v10 = pricing `monthly_prices.start_date`, since superseded — pricing is back to 1st-of-month; v11 = `receipts.payment_method` + the synced `settlements` table for the accountant wallet; `_createSyncInfra` skips not-yet-created tables so multi-step upgrades don't fail). Earlier note kept for history:
