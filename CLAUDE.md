...
## Gotchas
- The suite is green: `flutter test` (90) + `cd backend && npm test` (131). `test/widget_test.dart` holds real translation-parity tests — **every new string must be added to BOTH `en_US` and `ar_AR` maps** in `lib/utils/translations.dart` or the suite fails.
- ⚠️ **SQLite is `version: 12`** (v10 = pricing `monthly_prices.start_date`, superseded — pricing is back to 1st-of-month; v11 = `receipts.payment_method` + the synced `settlements` table for the accountant wallet; v12 = `settlements.method` ('cash'|'card') for the dual cash/card wallet; `_createSyncInfra` skips not-yet-created tables so multi-step upgrades don't fail; `DbHelper.wipeAllTables()` powers the hard-logout full wipe). Earlier note kept for history:
